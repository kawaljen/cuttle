...
module.exports = {
  ...
  cleanCardIdentifier: function (stringCard){
    ...
    // Cards with attachement eg : ‘TH(JH-p0,JC-p1,JD-p0)’
    else if(str.length > 2){
        const cardTemp=[];
        // get content inside parentheses
        const regex = /(?<=\().*?(?=\))/;
        const insideParenthese = regex.exec(str);
        //split using the comma
        const insideParentheseArray = insideParenthese.split(',');
        //then inside blocks by comma, split using the -, taking the part before the '-'
        //no comma
        if(insideParentheseArray.length === 1 && insideParentheseArray[0] === insideParenthese){
          cardTemp.push(insideParenthese.split('-')[0]);
        } else {
          insideParentheseArray.forEach(element => {
            cardTemp.push(element.split('-')[0]);
          });
        }
      
        // get content before parentheses
        cardTemp.push( stringCard.replace(/ \([\s\S]*?\)/g, '') );

        // Checks ...
        cardTemp.forEach(element => {
          if(element.split('').lenght === 2){
            cards.push(element.split('-')[0]);
          }
        });
    }
  ...
  convert:function (str){
    const cards = this.cleanCardIdentifier(str);

    cards.forEach(el=>{
        str = el.split('');
        const stringToCard = this.getRankAndSuit({suit : str[0], rank : str[1]});
        const newcard = new Card ({  id: el, 
